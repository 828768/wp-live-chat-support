...
/** running this in Pro and Basic for backweards compatibility (old basic didnt have this and new pro had it at the time) */
...
add_action("wplc_hook_settings_page_more_tabs","wplc_pro_hook_settings_page_more_doc_suggestions",9);
/**
 * Adds 'Doc Suggestions' content to settings area
 * @return void
 */
function wplc_pro_hook_settings_page_more_doc_suggestions() {
	remove_action("wplc_hook_settings_page_more_tabs","wplc_hook_settings_page_more_doc_suggestions");
    ...
    ?>
		<div id="tabs-doc-suggest">
            ...
            <?php wplc_pro_doc_suggestions_version_notice(); ?>
 ...
 * @return void
*/
function wplc_pro_doc_suggestions_version_notice(){
    global $wplc_version;
    if (intval(str_replace(".","",$wplc_version)) < 7000) {
        echo "<div class='update-nag' style='padding-top:0px;margin-top:5px;margin-bottom:10px;'>";
            echo "<p>In order to use Documentation Suggestions, please ensure you are using the latest basic version (version 7.0.00 or newer).</p>";
...
add_action('wplc_hook_admin_settings_save','wplc_doc_sugg_save_settings');
/**
 * Save settings hook for the documentation suggestions settings area
 * @return void
 */
function wplc_doc_sugg_save_settings() {
...
add_action("wplc_hook_message_sent","wplc_pro_filter_control_message_control",10,1); 
/**
 ...
 * @return void
 */
function wplc_pro_filter_control_message_control($data) {
	...
	if (isset($wplc_doc_sugg_data['wplc_enable_doc_suggestions']) && $wplc_doc_sugg_data['wplc_enable_doc_suggestions'] == '1') {
		$string = wplc_tokenise($data['msg'],'string');
		$original_string = $data['msg'];
		$string = wplc_remove_stop_words($string);
		...
		if($cdata->agent_id == 0)
		{
			...
			if (function_exists("wplc_record_chat_notification")) {
				...
				$returned_data = wplc_documentation_find($original_string);
				if ($returned_data) {
					...
					if(filter_var($cid, FILTER_VALIDATE_INT) || (is_string($cid) && preg_match('/^\d+$/', $cid))) {
					...
					$form_data = array(
						'chat_id' => $cid,
						'notification_text' => $formatted_msg
					);
					
					$result = wplc_node_server_post("user_chat_notification", $form_data);
				}
			}
		}
	}
...
add_filter("wplc_pro_filter_doc_suggestion_html","wplc_pro_filter_control_doc_suggestion_html",10,2);
/**
 ...
 */
function wplc_pro_filter_control_doc_suggestion_html($content,$returned_data) {
    $dir = dirname(dirname(__FILE__));
    $template_content_template = file_get_contents($dir."/templates/documentation-suggestion.html");
    $template_content_list_template = file_get_contents($dir."/templates/documentation-suggestion-item.html");
 ...
 */
function wplc_documentation_find($string) {
     ...
     */
    $search_strings = wplc_tokenise(wplc_remove_stop_words($string));
    ...
    foreach ($search_strings as $string) {
        ...
        if ( $my_query->have_posts() ) {
    	    ...
    	    while ( $my_query->have_posts() ) {
    	        ...
    	        $description = wplc_tokenise(get_the_content(),'string');
                $tmp_title = get_the_title();
                
                $lev_title = levenshtein(wplc_tokenise($tmp_title,'string'), $original_string);
                ...
                similar_text(wplc_tokenise($tmp_title,'string'), $original_string,$title_proximity);
                similar_text(wplc_tokenise($description,'string'), $original_string,$desc_proximity);
 ...
 */
function wplc_remove_stop_words($string) {
 ...
 */
function wplc_tokenise($string,$method = 'array') {
...
add_action("wplc_hook_chat_notification","wplc_pro_filter_control_chat_notification_documentation_suggestion",10,3);
/**
 ...
 * @return void
 */
function wplc_pro_filter_control_chat_notification_documentation_suggestion($type,$cid,$data) {
