 ...
 * @return void
*/
add_action("wplc_pro_update_db_hook", "wplc_pro_update_db_department", 10);
function wplc_pro_update_db_department(){
	global $wpdb;
	global $wplc_tblname_chat_departments;
...
add_filter("wplc_pro_agent_list_before_button_filter", "wplc_pro_agent_list_department_span", 10, 2);
/**
 * Adds current users assigned department to the agent information card
 *
 * @return string (html)
*/
function wplc_pro_agent_list_department_span($content, $user){
	$user_department = get_user_meta($user->ID, "wplc_user_department", true);
	if($user_department){
		$selected_department = wplc_get_department(intval($user_department));
...
add_action("wplc_pro_custom_user_profile_field_after_content_hook", "wplc_pro_department_user_profile_departments", 10, 1);
/**
 * Outputs department options to the user profile area
 *
 * @return void
*/
function wplc_pro_department_user_profile_departments($user){
	...
	if($user_is_agent == "1"){
		...
		if(current_user_can('manage_options', array(null))){
			...
			<?php
				...
				_e("Departments are not currently supported with the Cloud Server of WP Live Chat Support.", "wplivechat");
				?></p></div><?php
				}
				}
			?>
...
add_action("wplc_pro_set_user_hook", "wplc_pro_department_save", 10, 1);
/**
 * Handles the saving of user department data
 *
*/
function wplc_pro_department_save($user_id){
 ...
 * @return void
*/
add_action("wplc_hook_menu_mid","wplc_pro_department_menu",10,1);
function wplc_pro_department_menu($cap){
	add_submenu_page('wplivechat-menu', __('Departments', 'wplivechat'), __('Departments', 'edit_posts'), $cap[0], 'wplc-pro-departments', 'wplc_pro_departments_page');
}
 ...
 * @return void
*/
function wplc_pro_departments_page(){

	wplc_enqueue_admin_styles();
	$wplc_add_department_btn = isset($_GET['wplc_action']) ? "" : "<a href='?page=wplc-pro-departments&wplc_action=add_department' class='wplc_add_new_btn'>". __("Add New", 'wp-livechat') ."</a>";
	...
	$wplc_basic_version = str_replace(".", "", $wplc_version);
	
	$wplc_basic_version = intval( $wplc_basic_version );

	if( $wplc_basic_version < 7000 ){
		echo sprintf( "<div class='error'><p>".__("WP Live Chat Support Pro requires WP Live Chat Support Version 7.0.0 or greater in order for departments to function as expected. Please update WP Live Chat Support %s", "wplivechat")."</p></div>", " <a class='button button-secondary' href='".admin_url('update-core.php')."'>".__("Update now", "wplivechat")."</a>" );
	}
	...
	if(function_exists("wplc_cloud_load_updates")){
   	...
   	if(isset($_GET['wplc_action']) && ($_GET['wplc_action'] == "add_department" || $_GET['wplc_action'] == "edit_department")){
		$content .= wplc_pro_get_add_department_content();
    } else if(isset($_GET['wplc_action']) && ($_GET['wplc_action'] == "delete_department")){
    	$content .= wplc_pro_delete_department_content();
    } else {
    	$content .= wplc_pro_get_departments_table(); 	
    }
...
/**
 * Checks if user is using version 7 of WP Live Chat Support. 
 * If Not, Show a notice
 *
 * @return void
*/
function wplc_pro_department_version_notice(){
	global $wplc_version;
    if (intval(str_replace(".","",$wplc_version)) < 7000) {
    	echo "<div class='update-nag' style='padding-top:0px;margin-top:5px;margin-bottom:10px;'>";
  			echo "<p>In order to use Departments, please ensure you are using the latest basic version (version 7.0.00 or newer).</p>";
 ...
 * @return string (html)
*/
function wplc_pro_get_departments_table(){
	$content = "";

  	$results = wplc_get_all_deparments();
    ...
    $content .= 		"</tr>";
  	...
  	if($results){
  		foreach ($results as $result) {
  			$department_actions = "<a class='button' href='?page=wplc-pro-departments&wplc_action=edit_department&department_id=".$result->id."'>".__("Edit", "wp-livechat")."</a> ";
  			$department_actions .= "<a class='button' href='?page=wplc-pro-departments&wplc_action=delete_department&department_id=".$result->id."'>".__("Delete", "wp-livechat")."</a> ";
 ...
 * @return object 
*/
function wplc_get_all_deparments(){
	global $wpdb;
    global $wplc_tblname_chat_departments;
 ...
 * @return string (html)
*/
function wplc_pro_get_add_department_content(){
	$content = "";

	//Content Vars
	$department_name = "";

	$header_array = wplc_pro_department_admin_head();
	...
	$content .= "<table class=\"wp-list-table widefat fixed form-table\" cellspacing=\"0\" style='width:50%'>";
    ...
    $content .= 	"<tr>";
    $content .= 		"<td></td>";
    $content .=         "<td><input class='button button-primary' type='submit' name='wplc_department_submit' value='".__($wplc_submit_label, "wp-livechat")."'> <a href='".admin_url()."admin.php?page=wplc-pro-departments"."' class='button'>".__("Cancel", "wp-livechat")."</a></td>";
    $content .= 	"</tr>";
 ...
 * @return array
*/
function wplc_pro_department_admin_head(){
	if(isset($_GET['wplc_action'])){
		...
		if($_GET['wplc_action'] == "add_department"){
				if($form_valid && isset($_POST['wplc_department_submit'])){
					//All good continue
					if(wplc_add_department($return_array)){
						//Redirect here
						echo "<script> window.location = '".admin_url()."admin.php?page=wplc-pro-departments"."';</script>";
					}
				} else {
			...
			if($form_valid && isset($_POST['wplc_department_submit'])){
				...
				if(isset($_GET['department_id'])){
					if(wplc_edit_department($return_array, intval($_GET['department_id']))){
						//Show edit message
			...
			$data = wplc_get_department(intval($_GET['department_id']));
			if($data){
			...
			if(isset($_GET['department_id'])){
				...
				if(isset($_POST['delete_confirm'])){
					//Delete now
					if(wplc_delete_department(intval($_GET['department_id']))){
						//Success
					}
					echo "<script> window.location = '".admin_url()."admin.php?page=wplc-pro-departments"."';</script>";
				}
 ...
 * @return boolean
*/
function wplc_add_department($data){
	global $wpdb;
    global $wplc_tblname_chat_departments;
	if($data){
		$data_name;
 ...
 * @return boolean
*/
function wplc_edit_department($data, $id){
	global $wpdb;
    global $wplc_tblname_chat_departments;
	if($data){
		$department_name;
...
*/
function wplc_get_department($id){
	global $wpdb;
    global $wplc_tblname_chat_departments;
...
/**
 * Returns confirmation form prior to deleteing a department
 *
 * @return string (html)
*/
function wplc_pro_delete_department_content(){
	$header_array = wplc_pro_department_admin_head();
	$department_name = "";
	if($header_array){
	...
	if( (isset($_GET['wplc_action']) & isset($_GET['department_id'])) && ($_GET['wplc_action'] == "delete_department" && $_GET['department_id'] != "")){
		...
		$content .= 				" <a href='".admin_url()."admin.php?page=wplc-pro-departments' class='button'>".__("Cancel", "wp-livechat")."</a>";
		$content .= 			"</td>";
		$content .= 		"</tr>";
	  	$content .= 	"</table>";
	  	$content .= "</form>";
 ...
 * @return boolean
*/
function wplc_delete_department($id){
	global $wpdb;
	global $wplc_tblname_chat_departments;
	$id = intval($id);
...
add_filter("wplc_filter_setting_tabs","wplc_pro_department_settings_tab_heading");
/**
 * Creates settings area tab
 * 
 * @return string (html)
*/
function wplc_pro_department_settings_tab_heading($tab_array) {
...
add_action("wplc_hook_settings_page_more_tabs","wplc_pro_department_settings_tab_content");
/**
 * Creates settings area content
 * 
 * @return string (html)
*/
function wplc_pro_department_settings_tab_content() {
    $wplc_pro_settings = get_option("WPLC_PRO_SETTINGS");
    $selected_department = isset($wplc_pro_settings['wplc_default_department']) ? $wplc_pro_settings['wplc_default_department'] : -1;

    $allow_department_selection = isset($wplc_pro_settings['wplc_allow_department_selection']) ? intval($wplc_pro_settings['wplc_allow_department_selection']) : 0;
    ?>
	<div id="tabs-departments">
		<h3><?php _e("Departments", "wplivechat") ?></h3>
		<?php wplc_pro_department_version_notice(); ?>
		<?php 
            if(function_exists("wplc_cloud_load_updates")){
                $wplc_settings = get_option("WPLC_SETTINGS");
                if(!isset($wplc_settings['wplc_use_node_server']) || $wplc_settings['wplc_use_node_server'] != 1){
				...
				<tr>
					...
					<td>
						<select id="wplc_default_department" name="wplc_default_department"> 
	                    	...
	                    	?>
	                    </select> <a href="<?php echo admin_url('admin.php?page=wplc-pro-departments'); ?>" class="button button-secondary" title="<?php __('Create or Edit Departments')  ?>"><i class="fa fa-pencil wplc_light_grey"></i></a>
					</td>
				</tr>
		...
		<p><?php echo sprintf(__("Create departments <a href='%s'>here</a>."),admin_url('admin.php?page=wplc-pro-departments')); ?></p>
		<br>
...
add_filter("wplc_pro_setting_save_filter", "wplc_pro_department_settings_save", 10, 1);
/**
...
*/
function wplc_pro_department_settings_save($settings){
...
add_filter("wplc_alter_chat_list_sql_before_sorting", "wplc_pro_add_department_sql", 10, 1);
/**
 ...
 * @return string 
*/
function wplc_pro_add_department_sql($content){
...
add_action("wplc_after_chat_visitor_count_hook", "wplc_user_department_label");
/**
 * Outputs a span which shows the users department in the dashboard
 *
 * @return void
*/
function wplc_user_department_label(){
	$user_id = get_current_user_id();
	$user_department = get_user_meta($user_id ,"wplc_user_department", true);
 ...
 * @return void
*/
function wplc_dashboard_department_selector(){
	$content = "";
	$departments = wplc_get_all_deparments();
    ...
    echo $content;
    wplc_dashboard_department_selector_script();
}
 ...
 * @return void
*/
function wplc_dashboard_department_selector_script(){
	?>
	<script>
...
add_filter("wplc_log_user_on_page_insert_filter", "wplc_log_user_default_department", 10, 1);
/**
 ...
 * @return array
*/
function wplc_log_user_default_department($insert_data){
	$wplc_pro_settings = get_option("WPLC_PRO_SETTINGS");
    $selected_department = isset($wplc_pro_settings['wplc_default_department']) ? $wplc_pro_settings['wplc_default_department'] : -1; 
...
add_filter("wplc_start_chat_user_form_after_filter", "wplc_allow_department_selection_user_form", 10, 1);
/**
 ...
 * @return string
*/
function wplc_allow_department_selection_user_form($content){
	$wplc_pro_settings = get_option("WPLC_PRO_SETTINGS");
 	$allow_department_selection = isset($wplc_pro_settings['wplc_allow_department_selection']) ? intval($wplc_pro_settings['wplc_allow_department_selection']) : 0;
 	$default_department = isset($wplc_pro_settings['wplc_default_department']) ? $wplc_pro_settings['wplc_default_department'] : -1; 
...
add_action("wplc_start_chat_hook_after_data_insert", "wplc_pro_department_user_selected_department", 10, 1);
/**
 ...
 * @return void
*/
function wplc_pro_department_user_selected_department($cid){
	if(isset($_POST['wplc_user_selected_department'])){
		wplc_pro_department_update_department($cid, $_POST['wplc_user_selected_department']);
	} else {
		wplc_pro_department_update_department($cid, null);
	}
}
 ...
 * @return void
*/
function wplc_pro_department_update_department($cid, $department_id){
	global $wpdb;
	global $wplc_tblname_chats;
	$cid = intval($cid);
